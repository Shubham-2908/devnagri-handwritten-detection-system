...
class Model: 
	...
	def setupRNN(self):
		...
		# Create forward and backward cells - use default names to match checkpoint
		cells = [tf.nn.rnn_cell.LSTMCell(num_units=numHidden, state_is_tuple=True) for _ in range(2)]
	...
	def setupTF(self):
		...
		modelDir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'model')
		modelDir = os.path.normpath(modelDir) + os.sep
	...
	def save(self):
		"save model to file"
		self.snapID += 1
		modelDir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'model', 'snapshot')
		modelDir = os.path.normpath(modelDir)
		self.saver.save(self.sess, modelDir, global_step=self.snapID)
